...
def generate_pdf_report(job_data: dict, filename: str):
    ...
    pdf.cell(0, 10, f"Target: {job_data.get('target')}", 0, 1)
    ...
    if ports:
        ...
        for p in ports:
            ...
            pdf.cell(60, 8, str(p.get('service'))[:25], 1)
            pdf.cell(70, 8, str(p.get('product', '') + ' ' + p.get('version', ''))[:35], 1)
            pdf.ln()
        pdf.ln(10)
    ...
    if vulns:
        ...
        for v in vulns:
            ...
            # Simple cleanup of HTML tags if present
